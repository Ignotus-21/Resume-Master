...
// tests (via tests/fixtures/profile.js) AND by scripts/warmTectonicCache.js
// at Docker image build time — it lives in shared/, not tests/, because the
// image excludes tests/ (see .dockerignore) and the build-time compile
// smoke test must still have a full-coverage profile to render.
// `special` is the same shape with LaTeX special characters packed into
// every field — the whole-document escaping regression test.
const profile = {
  user: {
    name: 'Ada Lovelace',
